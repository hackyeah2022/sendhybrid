      ...
      <ActionsWrapper>
        <SingleActionsWrapper>
          ...
          <Link href={routes.SEND}>
            <SingleActionSmallButton>
              Wyślij nowy dokument
            </SingleActionSmallButton>
          </Link>
        </SingleActionsWrapper>
        {noOfRowsSelected > 0 && (
          <BulkActionsWrapper>
            <SmallButton onClick={handleGenerateSummaryReportClick}>
              Wygeneruj raport zbiorczy
            </SmallButton>
          </BulkActionsWrapper>
        )}
      </ActionsWrapper>
                ...
                {row.getVisibleCells().map(cell => {
                  return (
                    <td
                      key={cell.id}
                      style={{
                        width: cell.column.columnDef.meta?.fullWidth
                          ? '100%'
                          : 'auto',
                      }}
                    >
