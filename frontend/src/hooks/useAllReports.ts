...
export const getAllReportsQueryKey = (caseNumber?: string) => [`all-reports`, caseNumber ?? '']

const transformRes = (res) => res.map((singleDoc) => ({
    id: singleDoc.id,
...
const useAllReports = (caseNumber?: string) => useQuery(getAllReportsQueryKey(caseNumber), () => fetchAllReports(caseNumber), {
